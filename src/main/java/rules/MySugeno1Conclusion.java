 ...
 */
public class MySugeno1Conclusion implements Serializable{
    ...
    private Sugeno1FuzzyConclusion sugeno;
    ...
    public MySugeno1Conclusion(){
        List<Sugeno1Rule> rules = new ArrayList<Sugeno1Rule>();
        for (int i = 0; i < 8; i++) {
            ...
            rules.add(new Sugeno1Rule(newTerms,c[i]) {
                ...
                public String getName() {
                    ...
                    StringBuilder sb = new StringBuilder("Если x1 = ").append(getTerms().get(0).getName())
                            ...
                            .append(" тогда С = ");
                    if (k1 != null) {
                        sb.append(String.valueOf(k1[0])+" + ")
                                .append(String.valueOf(k1[1])).append("x1 + ")
                                .append(String.valueOf(k1[2])).append("x2 + ")
                                .append(String.valueOf(k1[3])).append("x3");
                    }
        ...
        sugeno = new Sugeno1FuzzyConclusion(rules);
    }
    ...
    public void correct(double[]x,double[] sigma,double trueValue,double sigmaTrueValue){
        ...
        double[][] backupC = new double[sugeno.getRules().size()][x.length];
        double[][][] backupArgs = new double[sugeno.getRules().size()][3][2];
        for (int i = 0; i < backupC.length; i++) {
            Sugeno1Rule rule = (Sugeno1Rule)sugeno.getRules().get(i);
        ...
        while (Math.abs(dz) > 0.01  && n < 1000 && m < 10000) {
            Conclusion conclusion = conclusions.pollFirst();
            double[] inputPoint = conclusion.getInputPoint();
            ...
            for (AbstractRule arule : sugeno.getRules()) {
                ...
                double[] k1 = rule.getRuleExpertValue();
                for (AbstractRule arule1 : sugeno.getRules()) {
                    Sugeno1Rule rule1 = (Sugeno1Rule) arule1;

                    double[] k2 = rule1.getRuleExpertValue();
                    double c1 = 0;
                    double c2 = 0;
                    for (int i = 1; i < k1.length; i++) {
                        c1 += inputPoint[i-1]*k1[i];
                        c2 += inputPoint[i-1]*k2[i];
                    }
                    c1 += k1[0];
                    c2 += k2[0];
                    sum1 += winTerm.getAlpha() * ( c1 - c2 );
                }
                ...
                double[] dc = new double[inputPoint.length];
                for (int i = 1; i < k1.length; i++) {
                    dc[i-1] = - (ETA/100 )* dz * winTerm.getAlpha() / sa * inputPoint[i-1];
                    k1[i] += dc[i-1];
                }
                k1[0] += - (ETA) * dz * winTerm.getAlpha()/sa;

                rule.setRuleExpertValue(k1 );
            }
